...
function get_metadata(id) {
    ...
    .then(({ metadata }) => {
      ...
      Object.entries(subjectData).forEach(([key, value]) => {
        ...
        tr.append('td').html(`<span class="panel-values">${value}</span>`).style("text-align", "left")
        .style("width", "220px");
      });
    });
  };
    ...
    d3.json(url)
      .then(data => {
        ...
        Plotly.newPlot('bar', [barTrace], {
          title: {text: `Top 10 Operational Taxonomic Units`,font: { size: 16, color: '#795609', family: 'calibri'}},
          height: 400,
          width: 600,
          xaxis: {
            tickwidth: 10,
